...
namespace CultureEventsBot.API.Core.HttpCommands
{
	public class ShowPlacesHttpCommand : HttpCommand
	{
		...
		public override async Task ExecuteAsync(IHttpClientFactory httpClient, Message message, TelegramBotClient client, DataContext context, int pageSize = 1)
		{
			...
			var placesIds = await HttpWork<Parent>.SendRequestAsync(HttpMethod.Get, $" https://kudago.com/public-api/v1.4/places/?lang={ConvertStringToEnum(user.Language)}&location=kzn&page_size={pageSize}&page={page}{categories}{actualSince}{actualUntil}", httpClient);
			if (placesIds != null)
			{
